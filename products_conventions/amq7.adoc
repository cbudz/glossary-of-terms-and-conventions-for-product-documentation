...
[[live-only]]
==== live-only (noun)
*Description*: A broker high availability policy that enables you to scale down brokers. If a live-only broker is shut down, its messages and transaction state are copied to another live broker.
