...
==== crash (verb)
[[crash]]
*Description*: When a program crashed, it means that is terminated unexpectedly. The IBM Style Guide suggests to use a more specific term, such as "fail". However, in Red Hat documentation is acceptable to use crash in certain cases: When writing errata descriptions, it's possible to use crash instead of terminate unexpectedly if terminate unexpectedly was used in a previous sentence. For example: An utility terminated unexpectedly because of a bug in the underlying source code. With this update, the utility no longer crashes.
