...
==== set up 
[[set-up-verb]]
*Description*: To install and configure hardware or software. Use the two-word form "set up" as a verb.

*Use it*: yes

*Class*: verb

*Incorrect forms*: set up as a noun

*See also*: xref:setup-noun [setup]

[discrete]
==== setup 
[[setup-noun]]
*Description*: The installation and configuration of hardware or software. Use the single-word form "setup" as a noun.

*Use it*: yes

*Class*: noun

*Incorrect forms*: setup as a verb

*See also*: xref:set-up-verb [set up]
...
SHA stands for Secure Hash Algorithm and is a cryptographic hash function. The encryption hash used in SHA-2 is significantly stronger and not subject to the same vulnerabilities as SHA-1.
SHA-2 variants are often specified using their digest size, in bits, as the trailing number, in lieu of "2." "SHA-224," "SHA-256," "SHA-384," and "SHA-512" are all correct when referring to these specific hash functions. 
