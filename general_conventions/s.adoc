...
*Incorrect forms*: server-side used as a noun

*See also*: xref:server-side-adjective [server-side]

⁠server-side/server side
    See "client-side/client side." 
    Operations that are performed by the client in a client-server relationship. Use the one-word form as an adjective. For example, "This is a client-side service."
