...
[[sudo]]
==== sudo (noun)
*Description*: sudo is a command that allows a user to run a program as another user (root user by default). Using the phrase 'as the root user' prior to a command is preferred over prefixing commands with `sudo` where a user requires elevated privillages (where reasonable).
